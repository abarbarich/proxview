...
import type { ChannelType, EmailConfig, NotifyMessage, SlackConfig, TelegramConfig } from './types.js';
...
/** Fan out a message to every enabled channel; per-channel failures are logged, not thrown. */
export async function dispatch(msg: NotifyMessage): Promise<void> {
  await Promise.all(
    listEnabledChannels().map(async (c) => {
      try {
